...
namespace brtr
{
	...
	{
		std::shared_ptr<buffer_opencl> buffer = std::dynamic_pointer_cast<buffer_opencl>(buf);
	...
	void kernel_opencl::set_kernel_arg(buffer_operation operation, int index, image_buffer buf)
	{
		...
		case buffer_operation::write:
			...
			break;
		case buffer_operation::read:
			m_queue.enqueueReadBuffer(m_buffers[buf.id], CL_TRUE, 0, buf.size*buf.stride, buf.data);
			break;
	...
	void kernel_opencl::run()
	{
		...
		{
			m_queue.enqueueWriteBuffer(buffer->m_buffer, CL_TRUE, 0, buffer->size * buffer->stride, buffer->data);
		}
		if (m_local_range[0] == 0)
		{
			m_queue.enqueueNDRangeKernel(m_kernel, cl::NullRange, m_global_range, cl::NullRange, NULL);
		}
		else
		{
			m_queue.enqueueNDRangeKernel(m_kernel, cl::NullRange, m_global_range, m_local_range, NULL);
		}
		
		for (const auto& buffer : m_buffers_to_read)
		{
			m_queue.enqueueReadBuffer(buffer->m_buffer, CL_TRUE, 0, buffer->size * buffer->stride, buffer->data);
		}
